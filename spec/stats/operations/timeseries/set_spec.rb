...
RSpec.describe Trifle::Stats::Operations::Timeseries::Set do
  ...
  describe '#perform' do
    ...
    it 'calls driver.set with correct parameters' do
      expect(mock_driver).to receive(:set) do |args|
        keys = args[:keys]
        ...
        expect(args[:values]).to eq({ count: 10, status: 'active' })
      end
    ...
    it 'passes through all values to driver' do
      complex_values = { count: 100, status: 'complete', metadata: { source: 'api', version: 2 } }
      operation = described_class.new(key: key, at: at_time, values: complex_values, config: mock_config)

      expect(mock_driver).to receive(:set).with(
        keys: anything,
        values: {
          count: 100,
          status: 'complete',
    ...
    it 'handles zero and false values correctly' do
      special_values = { count: 0, enabled: false, ratio: 0.0 }
      operation = described_class.new(key: key, at: at_time, values: special_values, config: mock_config)
