...
RSpec.describe Trifle::Stats::Transponder::Ratio do
  ...
  describe '#transpond' do
    context 'with standard parameters' do
      it 'calculates percentage ratio and adds it to the series data' do
        ...
        series.transpond.ratio(left: 'conversions.sample', right: 'conversions.total', response: 'conversions.ratio')
      ...
      it 'handles different path targets independently' do
        series.transpond.ratio(left: 'clicks.sample', right: 'clicks.total', response: 'clicks.ratio')
    ...
    context 'with custom key name' do
      it 'uses custom key for the calculated ratio' do
        series.transpond.ratio(left: 'conversions.sample', right: 'conversions.total', response: 'conversions.percentage')
      ...
      it 'uses custom sample and total field names' do
        ...
        custom_series.transpond.ratio(left: 'metrics.successes', right: 'metrics.attempts', response: 'metrics.ratio')
    ...
    context 'with root path (potential bug scenario)' do
      ...
      it 'handles empty path correctly' do
        root_series.transpond.ratio(left: 'sample', right: 'total', response: 'ratio')
    ...
    context 'with nested paths' do
      ...
      it 'calculates ratio for deeply nested paths' do
        nested_series.transpond.ratio(left: 'analytics.marketing.conversions.sample', right: 'analytics.marketing.conversions.total', response: 'analytics.marketing.conversions.ratio')
      ...
      it 'handles nested sample and total field paths' do
        ...
        complex_series.transpond.ratio(
          left: 'metrics.successes.count', 
          right: 'metrics.attempts.count',
    ...
    context 'with missing data' do
      ...
      it 'skips calculation when sample is missing' do
        incomplete_series.transpond.ratio(left: 'conversions.sample', right: 'conversions.total', response: 'conversions.ratio')
      ...
      it 'handles completely missing path gracefully' do
        incomplete_series.transpond.ratio(left: 'nonexistent.sample', right: 'nonexistent.total', response: 'nonexistent.ratio')
    ...
    context 'with edge case values' do
      ...
      it 'handles zero sample correctly' do
        edge_series.transpond.ratio(left: 'metrics.sample', right: 'metrics.total', response: 'metrics.ratio')
    ...
    context 'with precision scenarios' do
      ...
      it 'handles decimal precision correctly' do
        precision_series.transpond.ratio(left: 'metrics.sample', right: 'metrics.total', response: 'metrics.ratio')
    ...
    context 'with multiple transponder calls' do
      it 'can apply multiple ratios to different paths' do
        series.transpond.ratio(left: 'conversions.sample', right: 'conversions.total', response: 'conversions.ratio')
        series.transpond.ratio(left: 'clicks.sample', right: 'clicks.total', response: 'clicks.click_rate')
  ...
  describe 'integration with Series' do
    ...
    it 'returns the modified series for chaining' do
      result = series.transpond.ratio(left: 'conversions.sample', right: 'conversions.total', response: 'conversions.ratio')
    ...
    it 'can be combined with other series operations' do
      ...
      series.transpond.ratio(left: 'conversions.sample', right: 'conversions.total', response: 'conversions.ratio')
      mean_ratio = series.aggregate.mean(path: 'conversions.ratio')
      
      # mean of [25.0, 20.0, 18.75, 16.666...] ≈ 20.104
      expect(mean_ratio.first).to be_within(0.1).of(20.1)
    end
    ...
    it 'can be combined with other transponders' do
      ...
      # Add sum and count fields to test divide transponder
      ...
      series.transpond.divide(left: 'conversions.sum', right: 'conversions.count', response: 'conversions.mean_value')
      
      first_conv = series.series[:values].first['conversions']
      expect(first_conv).to include('ratio' => 25.0)
      expect(first_conv).to include('mean_value')
    end
  end
end